...
export default function Card(props) {
    ...
    return (
        ...
        <div className={`max-w-sm flex flex-col items-center justify-around bg-white  rounded-lg shadow `+style.glass}>
               ...
               <div className='h-1/6'>
               <Link href={props.link}><button type="button" className={`text-white   font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 font-bold `+((props.link!='')?style.btn:' bg-gray-600')}>{(props.link!='')?"Click":"Coming soon"}</button></Link>
               </div>
            </div>
    )
}
